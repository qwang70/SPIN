...
def MQ3():
    ...
    FROM modelfacts_function f, subprogram sp, modelfacts_program p 
    WHERE f.program_id = p.program_id AND f.program_id != sp.subprogram_id;"""
    cursor.execute(query)
    results = cursor.fetchall()
    for r in results:
...
if __name__ == '__main__':
    DataName1 = "led_highlighter[thresholded_image]"
    DataName2 = "led_highlighter[formated_centers]"
    BlockName1 = "led_highlighter.leds_position"
    BlockName2 = "led_highlighter.extract_bright"
    BlockName3 = "led_highlighter.to_gray"
